...
def SendHELLO(sock :socket, info_hash,peer_id,bitfield=None): #when downloader send its without bitfield so default is None
    message = {
        "type": "HELLO",
        "info_hash": info_hash,
        "peer_id": peer_id
...
def Send_Give_Piece(sock : socket, bdata_content : bytes,info_hash : str,index :  int, block_size : int, current_offset : int):
